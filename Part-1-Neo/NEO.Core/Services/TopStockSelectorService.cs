...
namespace NEO.Core.Services
{
    public class TopStockSelectorService
    {
        ...
        public TopStockSelectorService(DatabaseHelper db, StockFilterService filter)
        {
            _db = db;
            _filter = filter;
        }
        ...
        // =============================================
        // SELECT TOP STOCKS — DB MODE
        // Reads from Table_1_All_Stocks
        // =============================================
        public async Task<List<Stock>> SelectTopStocksAsync(
            List<string> sectors,
            int topN = 10)
        {
            Console.WriteLine("\n🔵 STAGE 5 — Selecting Top Stocks (DB Mode)...");
            ...
            foreach (var sector in sectors)
            {
                ...
                var sectorStocks = await _db.LoadStocksBySectorAsync(sector);

                if (sectorStocks.Count == 0)
                {
                    Console.WriteLine($"   ⚠️ No DB stocks found for sector: {sector}");
                    continue;
                }

                Console.WriteLine($"   → Loaded {sectorStocks.Count} stocks from DB");

                var filtered = _filter.FilterStocks(sectorStocks, sector);

                if (filtered.Count == 0)
                {
                    Console.WriteLine($"   ⚠️ No stocks passed filters for sector: {sector}");
                    continue;
                }
                ...
                {
                    Console.WriteLine($"   Rank {s.Rank}: {s.Symbol,-12} " +
                                      $"1D: {s.Pct1d,6:F2}%  " +
                                      $"Score: {s.Score:F3}");
                }

                allStocks.AddRange(ranked.Take(topN));
            }
        ...
        // =============================================
        // MOCK MODE — Fallback if DB has no usable data
        // =============================================
            ...
            int topN = 10)
        {
            ...
            foreach (var sector in sectors)
            {
                ...
                if (mockStocks.Count == 0)
                {
                    Console.WriteLine($"   ⚠️ No mock stocks defined for sector: {sector}");
                    continue;
                }

                Console.WriteLine($"   → Filtering {mockStocks.Count} stocks for sector: {sector}");

                var filtered = _filter.FilterStocks(mockStocks, sector);
                var ranked = _filter.ScoreAndRankStocks(filtered);
